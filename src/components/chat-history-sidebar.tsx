...
function ChatHistorySidebarContent({
  isCollapsed = false,
  className,
}: ChatHistorySidebarContentProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        const chatsChanged = userChats.length !== chats.length || 
          (userChats.length > 0 && chats.length > 0 && 
        ...
        <div className="p-3 border-t border-[#02133B]/20">
          <div className="flex items-center gap-3 p-2 rounded-lg bg-[#E4EDFF]/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#02133B] truncate">
                {user.email}
              </p>
            </div>
            <Button
...
export default function ChatHistorySidebar({ 
  ...
  return (
    <>
      ...
      >
        <ChatHistorySidebarContent isCollapsed={isCollapsed} />
      </div>
